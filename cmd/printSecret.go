...
manually to your config file. You must login with 'seqtoid-cli login'
to obtain a secret. You can then use seqtoid-cli print-secret to
...
careful while handling it.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if !hasSecret {
			fmt.Println("no secret defined, try running 'seqtoid-cli login' to generate one")
		} else {
			fmt.Println(secret)
		}
	},
